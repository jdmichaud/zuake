...
const VM = struct {
  ...
  pc: usize,
  // List of entities
  entities: std.ArrayList(entityModule.Entity),
  ...
  pub fn getFieldPtr(self: Self, err: *RuntimeError, entityIndex: u32, fieldIndex: u32) !u32 {
    if (entityIndex >= self.entities.items.len) {
      _ = try std.fmt.bufPrint(&err.message, "entities index {} is out of bound (nb of entities: {})", .{
        entityIndex, self.entities.items.len,
      });
      return error.RuntimeError;
    }
    ...
    return intCast(u32, (entityIndex * self.maxFieldIndex) + fieldIndex);
  }

  // From an opaque field handler generated by getFieldPtr, retrieve the entity
  // index and field name.
  pub fn getFieldFromPtr(self: Self, err: *RuntimeError, fieldPtr: usize)
    !struct { entityIndex: usize, fieldName: []const u8 } {
    ...
    _ = try std.fmt.bufPrint(&err.message, "Invalid field index {}", .{
      fieldIndex,
    });
    return error.RuntimeError;
  }
  ...
  inline fn executeStatement(self: *Self, statement: datModule.Statement, err: *RuntimeError) !bool {
    ...
    switch (statement.opcode) {
      ...
      datModule.OpCode.STOREP_F => {
        ...
        const tmp = try self.getFieldFromPtr(err, self.mem32[fieldPtr]);
        const value = bitCast(f32, self.mem32[src]);
        try self.entities.items[tmp.entityIndex].put(tmp.fieldName,
          entityModule.EntityField{ .value = entityModule.EntityValue{ .Float = value } });
        self.pc += 1;
        return false;
      },
